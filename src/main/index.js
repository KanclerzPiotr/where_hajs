...
function createWindow() {
  ...
  // Handle close event - ask renderer if there are unsaved changes
...
app.whenReady().then(() => {
  ...
  // File system APIs
  ipcMain.handle('save-project-file', async (event, jsonData) => {
    try {
      const { filePath, canceled } = await dialog.showSaveDialog({
        title: 'Zapisz projekt',
        defaultPath: `wherehajs_${new Date().toISOString().slice(0, 10)}.json`,
